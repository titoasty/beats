...
BEATS.GUI = GUI = (function() {
  function GUI(beats) {
    var frameRate, grids, i, j, k, l, line, linesCount, ref, ref1, startTime;
    this.beats = beats;
    ...
    this.buffers = {
      music: this.beats.media.src
    };
    this.controls = document.querySelector('.controls');
    this.graphKeys = document.querySelector('.graph-keys');
    this.graphKeysCanvas = this.graphKeys.querySelector('canvas');
    this.keysByTime = [];
    this.currentKey = this.beats.keys[0];
    this.graphSpectrum = document.querySelector('.graph-spectrum');
    this.graphSpectrumCanvas = this.graphSpectrum.querySelector('canvas');
    this.track = document.querySelector('.track');
    this.trackCanvas = this.track.querySelector('canvas');
    this.track.addEventListener('mousedown', (function(_this) {
      return function(event) {
        return _this.moveTo(event);
      };
    })(this));
    ...
    for (i = k = 0, ref = grids.length; 0 <= ref ? k < ref : k > ref; i = 0 <= ref ? ++k : --k) {
      linesCount = (i + 1) * 20;
    ...
    loadBuffers(this.buffers, this.assets, (function(_this) {
      return function() {
        _this.buffer = _this.assets.buffers.music;
        return _this.updateTrack();
      };
    })(this));
  ...
  GUI.prototype.resample = function(width, audioData) {
  ...
  GUI.prototype.initializeOutputs = function() {
    var i, k, name, output, ref, results, template;
    this.outputs = [];
    this.values = [];
    this.levels = [];
    this.thresholds = [];
    ...
    results = [];
    for (i = k = 0, ref = this.beats.keys.length; 0 <= ref ? k < ref : k > ref; i = 0 <= ref ? ++k : --k) {
      output = template.cloneNode(true);
      output.className = "output";
      name = output.querySelector('.name');
      name.innerText = this.beats.keys[i].name;
      ...
      results.push(this.circles[this.circles.length++] = output.querySelector('.circle'));
    }
    return results;
  };

  GUI.prototype.initializeSequences = function() {
    var i, k, name, position, positions, ref, results, sequence, sequences, template, value;
    sequences = this.track.querySelector('.sequences');
    template = this.track.querySelector('.template');
    positions = this.beats.sequencer.positions;
    results = [];
    for (i = k = 0, ref = positions.length; 0 <= ref ? k < ref : k > ref; i = 0 <= ref ? ++k : --k) {
      position = positions[i] / this.media.duration;
      ...
      results.push(sequences.appendChild(sequence));
    }
    return results;
  };

  GUI.prototype.initializeKeys = function() {
    var i, k, key, name, ref, template;
    this.keys = [];
    template = this.graphSpectrum.querySelector('.template');
    for (i = k = 0, ref = this.beats.keys.length; 0 <= ref ? k < ref : k > ref; i = 0 <= ref ? ++k : --k) {
      key = template.cloneNode(true);
      this.keys[i] = key;
      name = key.querySelector('.name');
      name.innerText = this.beats.keys[i].name;
      this.graphSpectrum.appendChild(key);
    }
    return this.updateKeys();
  };

  GUI.prototype.moveTo = function(event) {
    var progress;
    progress = (event.clientX - this.track.offsetLeft) / this.track.clientWidth;
    return this.media.currentTime = progress * this.media.duration;
  };

  GUI.prototype.updateTrack = function() {
    var context, i, j, resampledData, results, width;
    if (this.buffer == null) {
      return;
    }
    width = this.trackCanvas.width;
    context = this.trackCanvas.getContext('2d');
    resampledData = this.resample(width, this.buffer.getChannelData(0));
    context.translate(0.5, this.trackCanvas.clientHeight * 0.5);
    context.scale(1, 100);
    i = 0;
    results = [];
    while (i < width) {
  ...
  GUI.prototype.updateOutputs = function() {
    var i, k, key, ref, results;
    results = [];
    for (i = k = 0, ref = this.beats.keys.length; 0 <= ref ? k < ref : k > ref; i = 0 <= ref ? ++k : --k) {
      ...
      results.push(this.values[i].innerText = key.value.toFixed(3));
    }
    return results;
  };

  GUI.prototype.updateKeys = function() {
    var height, i, k, key, ref, results, values, width, x, y;
    results = [];
    for (i = k = 0, ref = this.keys.length; 0 <= ref ? k < ref : k > ref; i = 0 <= ref ? ++k : --k) {
      key = this.keys[i];
      values = this.beats.keys[i];
      if (values.active) {
        key.className = "key active";
      } else {
        key.className = "key";
      }
      x = values.start * this.levelSize;
      y = (1 - values.max) * this.graphSpectrumCanvas.height;
      width = (values.end - values.start) * this.levelSize;
      height = (values.max - values.min) * this.graphSpectrumCanvas.height;
      key.style.height = height + 'px';
      key.style.width = width + 'px';
      results.push(key.style.transform = 'translate(' + x + 'px,' + y + 'px)');
    }
    return results;
  };

  GUI.prototype.updateProgress = function() {
    var progress;
    progress = this.media.currentTime / this.media.duration;
    this.progress.style.transform = 'translateX(' + progress * 100 + '%)';
    return this.time.innerText = this.media.currentTime.toFixed(3);
  };

  GUI.prototype.updateGraphKeys = function() {
    var context, height, i, k, ref;
    context = this.graphKeysCanvas.getContext('2d');
    context.clearRect(0, 0, this.graphSpectrumCanvas.width, this.graphSpectrumCanvas.height);
    context.strokeStyle = "#5AAAFF";
    if (this.keysByTime.length >= this.waveCountKeys) {
      this.keysByTime.shift();
    }
    if (this.currentKey == null) {
      return;
    }
    this.keysByTime[this.keysByTime.length++] = this.currentKey.value;
    context.beginPath();
    context.setLineDash([]);
    for (i = k = 0, ref = this.waveCountKeys; 0 <= ref ? k < ref : k > ref; i = 0 <= ref ? ++k : --k) {
      height = -this.keysByTime[i] * this.graphKeysCanvas.height + this.graphKeysCanvas.height;
      context.lineTo(i * this.waveSizeKeys, height);
    }
    context.stroke();
    ...
    height = -this.currentKey.threshold * this.graphKeysCanvas.height + this.graphKeysCanvas.height;
    context.setLineDash([5, 5]);
    context.moveTo(0, height);
    context.lineTo(this.graphKeysCanvas.width, height);
    return context.stroke();
  };

  GUI.prototype.updateGraphSpectrum = function() {
    var context, gap, halfHeight, height, i, k, l, offset, ref, ref1, width, x, y;
    context = this.graphSpectrumCanvas.getContext('2d');
    context.clearRect(0, 0, this.graphSpectrumCanvas.width, this.graphSpectrumCanvas.height);
    for (i = k = 0, ref = this.levelCount; 0 <= ref ? k < ref : k > ref; i = 0 <= ref ? ++k : --k) {
      context.fillStyle = '#5AAAFF';
      if (this.spectrum[i] > 0) {
        x = i * this.levelSize;
        height = this.output.spectrum[i] * this.graphSpectrumCanvas.height;
        offset = 0.2;
        gap = this.levelSize * offset;
        width = this.levelSize * (1 - offset);
        y = this.graphSpectrumCanvas.height - height;
        context.fillRect(x, y, width, height);
      }
    }
    context.beginPath();
    context.strokeStyle = "rgba(200, 200, 200, 0.95)";
    halfHeight = this.graphSpectrumCanvas.height * 0.5;
    for (i = l = 0, ref1 = this.waveCount; 0 <= ref1 ? l < ref1 : l > ref1; i = 0 <= ref1 ? ++l : --l) {
      height = this.output.waveform[i] * halfHeight + halfHeight;
      context.lineTo(i * this.waveSize, height);
    }
    return context.stroke();
  };

  GUI.prototype.updateGUI = function() {
    var Q, filter, frequency, gain, i, k, key, keyFolder, keysFolder, l, modulator, modulatorFolder, modulators, modulatorsFolder, modulatorsNeedsUpdate, output, ref, ref1, results, that;
    modulators = [null];
    modulatorsFolder = this.gui.addFolder('Modulators');
    for (i = k = 0, ref = this.beats.modulators.length; 0 <= ref ? k < ref : k > ref; i = 0 <= ref ? ++k : --k) {
      ...
      that = this;
      modulatorsNeedsUpdate = function(boolean) {
        var l, name, ref1, results;
        name = that.graphSpectrum.querySelector('.container-name span');
        results = [];
        for (i = l = 0, ref1 = that.beats.modulators.length; 0 <= ref1 ? l < ref1 : l > ref1; i = 0 <= ref1 ? ++l : --l) {
          if (this.object.name === that.beats.modulators[i].name) {
            if (boolean) {
              name.innerText = this.object.name;
              results.push(that.changeOutput(this.object.analyser));
            } else {
              that.changeOutput(that.beats.analyser);
              name.innerText = 'Main analyser';
              results.push(that.beats.modulators[i].output = false);
            }
          } else {
            results.push(void 0);
          }
    ...
    keysFolder = this.gui.addFolder('Keys');
    results = [];
    for (i = l = 0, ref1 = this.beats.keys.length; 0 <= ref1 ? l < ref1 : l > ref1; i = 0 <= ref1 ? ++l : --l) {
      key = this.beats.keys[i];
      keyFolder = keysFolder.addFolder(key.name);
      keyFolder.add(key, 'active').listen().onChange(this.updateKeys);
      keyFolder.add(key, 'start', 0, this.levelCount).listen().step(1).onChange(this.updateKeys);
      keyFolder.add(key, 'end', 0, this.levelCount).listen().step(1).onChange(this.updateKeys);
      keyFolder.add(key, 'min', 0, 1).listen().step(0.01).onChange(this.updateKeys);
      keyFolder.add(key, 'max', 0, 1).listen().step(0.01).onChange(this.updateKeys);
      keyFolder.add(key, 'modulator', modulators).listen().onChange(this.updateKeys);
      if (!i) {
        key.graph = true;
      } else {
        key.graph = false;
      ...
      that = this;
      results.push(keyFolder.add(key, 'graph').listen().onChange(function(value) {
        var m, name, ref2;
        for (i = m = 0, ref2 = that.beats.keys.length; 0 <= ref2 ? m < ref2 : m > ref2; i = 0 <= ref2 ? ++m : --m) {
          that.beats.keys[i].graph = false;
        }
        name = that.graphKeys.querySelector('.container-name span');
        if (value) {
          that.currentKey = this.object;
          name.innerText = this.object.name;
  ...
  GUI.prototype.update = function() {
    ...
    if (delta > this.interval) {
      this.oldTime = newTime - (delta % this.interval);
      this.updateGraphKeys();
    }
    this.spectrum = this.output.spectrum;
    this.waveform = this.output.waveform;
    if ((this.waveform != null) && (this.spectrum != null) && !this.beats.media.paused) {
      this.updateGraphSpectrum();
      this.updateOutputs();
  ...
  GUI.prototype.resize = function() {
    ...
    this.graphSpectrumCanvas.width = this.graphSpectrum.clientWidth;
    this.graphSpectrumCanvas.height = this.graphSpectrum.clientHeight;
    this.trackCanvas.width = this.track.clientWidth;
    this.trackCanvas.height = this.track.clientHeight;
    this.graphKeysCanvas.width = this.graphKeysCanvas.parentNode.clientWidth;
    this.graphKeysCanvas.height = this.graphKeysCanvas.parentNode.clientHeight;
    this.levelCount = this.beats.levelCount || this.beats.analyser.frequencyData.length;
    this.levelSize = Math.round(this.graphSpectrumCanvas.width / this.levelCount);
    this.waveCount = this.beats.analyser.timeDomainData.length;
    this.waveSize = this.graphSpectrumCanvas.width / this.waveCount;
    this.waveCountKeys = 300;
    this.waveSizeKeys = this.graphKeysCanvas.width / this.waveCountKeys;
...
},{"Beats":2,"dat-gui":5,"loadBuffers":4}],2:[function(require,module,exports){
var BEATS,
  bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; };
...
BEATS.Interface = (function() {
  function Interface(context, input, parameters) {
  ...
  Interface.prototype.stereoToMono = function(audioBuffer) {
    var i, leftChannel, mixedChannel, results, rightChannel;
    if (audioBuffer.numberOfChannels = 2) {
      ...
      while (i--) {
        ...
        results.push(leftChannel[i] = rightChannel[i] = mixedChannel);
      }
      return results;
    }
  };
...
audio.oncanplaythrough = function() {
  var beats, beatsGUI, frameRate, input, interval, oldTime, sequencer, sequences, startTime, update;
  ...
  sequences = [
    [
      ...
      54, {
        onStart: (function(_this) {
          return function() {
            return beats.get('hit-hat').active = true;
          };
        })(this)
      }
    ], [
...
},{"Beats":2,"Beats.GUI":1}],4:[function(require,module,exports){
module.exports = (function(_this) {
  return function(buffers, assets, callback) {
    var audioContext, keys, length, load, next;
    ...
    load = function(name) {
      ...
      request.onload = function() {
        return audioContext.decodeAudioData(request.response, function(decodedBuffer) {
